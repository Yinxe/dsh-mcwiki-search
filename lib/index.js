...
// settingsNamespace / installSettingsSection 两个辅助函数（0.1.1-rc.2 及以前有）。
// settingsNamespace 只是 kebab-case 运行时校验，本地内联同款实现；
// installSettingsSection 改用 settings 服务上的 installSection 方法（官方
// dsh-bash-local 同款写法：ctx.inject(['settings'], sctx => sctx.settings.installSection(...))）。
