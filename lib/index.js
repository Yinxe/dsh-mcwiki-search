 ...
 *
 * 以及两个同源 JSON 路由（设置页状态 / 连接测试）：
 *   GET  /ext/mcwiki/state
 *   POST /ext/mcwiki/test  { query, title?, lang?, limit? }
 */
...
/** 配置默认值（无 schemastery，手动合并，保证零依赖）。
...
function registerRoutes(ctx, config) {
  ...
  }), 'mcwiki-search: state route')

  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/ext/mcwiki/test',
    handler: async (req, res) => {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, error: 'forbidden' })
